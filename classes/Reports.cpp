...
void Reports::writeRecredenciamento(const string& diretorio, const vector<int>& dataRecredenciamento, PPGI& ufes) {
    ...
    for (const auto& docente : docentes) {
        ...
        vector<int> dataNascimento = docente.get_data_nascimento();
...
void Reports::writePublicacoes(const string& diretorio, PPGI& ufes) {
    ...
    vector<Publicacao> publicacoes = ufes.get_publicacoes();

    // Ordenar publicações

    /// provavelmente tem erro aqui
    sort(publicacoes.begin(), publicacoes.end(), [](const Publicacao &a, const Publicacao &b) {
